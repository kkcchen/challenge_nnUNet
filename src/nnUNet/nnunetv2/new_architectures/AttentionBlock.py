...
class AttentionBlock(nn.Module):
    def __init__(self,
        ...
        self.query_mem = nn.Linear(memory_size, memory_size, bias=False)
        self.key_mem   = nn.Linear(memory_size, memory_size, bias=False)
        self.value_mem = nn.Linear(memory_size, memory_size, bias=False)

        self.key_unet = nn.Linear(memory_size, memory_size, bias=False)
        self.value_unet = nn.Linear(memory_size, memory_size, bias=False)

        self.attention = nn.MultiheadAttention(embed_dim=memory_size, num_heads=1, dropout=0.0, batch_first=True)
    ...
    def forward(self, 
        ...
        key = torch.cat((key_mem, key_unet), dim=1)
        value = torch.cat((value_mem, value_unet), dim=1)

        # attention
        feature_map = self.attention(query_mem, key, value, need_weights=False)
